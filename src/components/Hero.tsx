...
export const Hero: React.FC<HeroProps> = ({ onSelectCreator, onOpenSignUp }) => {
  ...
  return (
    <section className="w-full px-2 sm:px-6 md:px-8 lg:px-10 pb-8 sm:pb-10 pt-1 sm:pt-2">
      {/* Dark Charcoal Hero Box */}
      <div className="hero-container rounded-[22px] sm:rounded-[36px] px-3.5 sm:px-10 lg:px-14 pt-6 sm:pt-9 md:pt-10 pb-6 sm:pb-9 md:pb-10 text-center text-white relative shadow-2xl overflow-visible border border-gray-800">
        
        {/* Animated Headline - Fully Responsive */}
        <motion.h1
          variants={headlineContainerVariants}
          initial="hidden"
          whileInView="visible"
          viewport={{ once: true, margin: '-20px' }}
          className="font-h1 text-center max-w-6xl mx-auto text-white text-2xl sm:text-4xl md:text-5xl lg:text-[52px] tracking-tight leading-tight sm:leading-tight px-1 sm:px-0"
        >
          <span className="inline-block text-white">
            <motion.span variants={wordFadeSlideVariants} className="inline-block mr-[0.2em] sm:mr-[0.25em]">
              Hire
            </motion.span>
            <motion.span variants={wordFadeSlideVariants} className="inline-block mr-[0.2em] sm:mr-[0.25em]">
              Top
            </motion.span>
            <motion.span variants={wordFadeSlideVariants} className="inline-block mr-[0.3em] sm:mr-[0.4em] hero-highlight">
              Creators.
            </motion.span>
            <motion.span variants={wordFadeSlideVariants} className="inline-block mr-[0.2em] sm:mr-[0.25em]">
              Get
            ...
            <motion.span
              ...
              className="inline-block font-serif-italic hero-highlight"
            >
              Fast.
            </motion.span>
          </span>
        ...
        {/* Subtitle */}
        <p className="hero-subtitle max-w-2xl mx-auto mt-2 sm:mt-2.5 text-xs sm:text-sm md:text-base text-gray-300 px-2">
          Find, book, and pay vetted UGC creators in minutes.
        </p>

        {/* White Pill Search Bar Container */}
        <div className="mt-5 sm:mt-7 w-full max-w-[1080px] mx-auto relative z-30">
          <form
            onSubmit={handleSearchSubmit}
            className="bg-white rounded-full p-1 sm:p-1.5 md:p-2 flex items-center shadow-2xl border border-gray-100 text-gray-900 transition-all focus-within:ring-2 focus-within:ring-[#9B87F5]/30 overflow-hidden"
          >
            ...
            <div className="h-4 sm:h-5 w-px bg-gray-200 mx-1 sm:mx-1.5 shrink-0" />
            ...
            <input
              ...
              placeholder="Search 500,000+ vetted creators (Brands only)..."
              className="min-w-0 flex-1 bg-transparent border-none text-xs sm:text-sm text-gray-900 placeholder-gray-400 focus:outline-none px-1.5 sm:px-2 font-medium"
            />

            {/* Search Action Button */}
            <button
              type="submit"
              className="search-btn font-semibold text-xs sm:text-sm px-3.5 sm:px-6 py-1.5 sm:py-2 rounded-full flex items-center gap-1.5 sm:gap-2 transition-all cursor-pointer shadow-md hover:shadow-lg active:scale-95 shrink-0 ml-1"
            >
        ...
        {/* Filter Pills Row - Exact same width as search bar */}
        <div className="flex flex-wrap lg:flex-nowrap justify-center lg:justify-between items-center gap-1.5 sm:gap-2 mt-3 sm:mt-4 w-full max-w-[1080px] mx-auto text-xs font-medium px-1">
            ...
            return (
              <button
                ...
                className={`group border rounded-full px-3 py-1.5 items-center gap-1.5 transition-all cursor-pointer select-none whitespace-nowrap shrink-0 ${
                  ...
                  isActive
                    ? 'bg-[#9B87F5]/30 text-white border-[#9B87F5] shadow-sm'
          ...
          {(selectedFilterId || searchQuery) && (
            <button
              ...
              className="text-[#9B87F5] hover:text-[#B4A5F8] text-xs font-semibold underline flex items-center gap-1 ml-1 cursor-pointer whitespace-nowrap shrink-0"
            >
        ...
        <div className="mt-6 sm:mt-7 w-full max-w-[1080px] mx-auto">
          {filteredCreators.length > 0 ? (
            <div className="flex flex-nowrap overflow-x-auto snap-x snap-mandatory gap-3.5 sm:gap-4 pb-4 pt-1 scroll-smooth no-scrollbar -mx-2 px-2 sm:-mx-4 sm:px-4 md:-mx-6 md:px-6 lg:mx-0 lg:px-0 lg:grid lg:grid-cols-4 lg:gap-4.5 lg:pb-0 lg:overflow-visible">
              {filteredCreators.map((creator) => (
                <div
                  key={creator.id}
                  className="w-[230px] sm:w-[260px] md:w-[280px] lg:w-full shrink-0 lg:shrink snap-start"
                >
                  <CreatorCard
                    creator={creator}
